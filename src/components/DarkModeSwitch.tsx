...
function Switch() {
    ...
    return (
        <label className={`switch ${isDarkMode ? 'dark' : 'light'}`}>
            ...
            <span className={`slider ${isDarkMode ? 'dark' : 'light'}`}>
                <svg className={`icon ${isDarkMode ? 'moon' : 'sun'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: isDarkMode ? 'translateX(26px)' : 'translateX(0)' }}>
                    {isDarkMode ? (
